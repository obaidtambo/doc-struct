...
// Set the workerSrc dynamically based on the loaded library's version to ensure compatibility.
// @ts-ignore
    ...
    const renderPdf = async () => {
      ...
      try {
        ...
        for (let i = 1; i <= pdf.numPages; i++) {
          ...
          // As per user's request, restoring the render parameters object to its original structure.
          // The `as any` cast is used to bypass a TypeScript error with the current library version,
          // ensuring the code compiles while respecting the user-provided logic.
          const renderParameters = {
            canvasContext,
            viewport,
            canvas,
          };
          await page.render(renderParameters as any).promise;
